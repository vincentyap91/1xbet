...
- **Sports / Line page** (`sports.html`, `data-page="sports"`): desktop Line sportsbook ([1xlite line](https://1xlite-733980.pro/en/line?platform_type=desktop)) — left sports nav (Sports tab default) · featured match rail (`.sp-top-rail`) · competition chips (`.sp-league-chips`) · **same odds table chrome as National Team** (`#live-events` te-toolbar / te-filters / `#live-table`, Line data via `js/script.js`) · right bet slip. Header **Sports** is a link to `sports.html` (click label) with dropdown caret for National Team / Big Tournaments / Long-term (no Sports item inside). ≤900: sticky `.nt-mobile-subbar` — back · **Sports** · search / favourites / filter (`data-mh-mf-open`).
- **Sports / Live subcategory strip** (`.sb-subnav`, ≤900 only): horizontal icon+label cards mirroring account `.acc-subnav` (withdraw). Injected by `js/auth-modals.js` `initSbSubnav` into `#main-content` **below** `.nt-mobile-subbar` when present. **Sports group** (`sports` / `national-team` / `big-tournaments` / `long-term-bets`): Sports · Bet on Your National Team · Bet on Big Tournaments · Long-term bets. **Live group** (`multi-live` / `live-national-team` / `marble-live` / `fast-bet`): Multi-LIVE · Bet on Your National Team · Marble-Live · Fast bet. Active card uses `--odds-hover` + `--section-blue` border. Hidden on `mobile/` and desktop (>900). Does not alter header dropdowns.
- **Live pages title bar** (`multi-live.html` / `marble-live.html` / `fast-bet.html`, ≤900): same sticky `.nt-mobile-subbar` as National Team — back · page title · search / favourites / filter. Desktop breadcrumbs (`.ml-toolbar` / `.te-crumbs` / `.fb-toolbar`) and Fast Bet page `h1.fb-title` are hidden on mobile.
